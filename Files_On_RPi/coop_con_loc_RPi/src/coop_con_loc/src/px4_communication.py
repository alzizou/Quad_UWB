...
def px4_communication():
	...
	publish_att_data = String()
	rospy.init_node("px4_communication", anonymous=True)
	rate = rospy.Rate(100)
	rospy.on_shutdown(clean_shutdown)
	pub_att = rospy.Publisher("px4_att", String, queue_size = 1)
...
#		PX4_connection.mav.vision_speed_estimate_send(PX4_ACC_msg.time_usec,vel_NED_mps[0],vel_NED_mps[1],vel_NED_mps[2],Vel_Covariance)
		...
		PX4_Att_msg = PX4_connection.recv_match(type='ATTITUDE',blocking=True)
		if not PX4_Att_msg:
			print(colored("PX4 attitude data: no message is received!","red"))
		else:
			Att_roll = PX4_Att_msg.roll
			Att_pitch = PX4_Att_msg.pitch
			Att_yaw = PX4_Att_msg.yaw
			print("PX4 Att data: %s" % PX4_Att_msg)
#			Results_File_ACC.write("%d %.6f %.6f %.6f\r\n" % (rospy.get_time(),Acc_m[0],Acc_m[1],Acc_m[2]))
			data_Att = {'time':time_stmp , 'roll':Att_roll , 'pitch':Att_pitch, 'yaw':Att_yaw}
			publish_att_data.data = json.dumps(data_Att)
			pub_att.publish(publish_att_data)
