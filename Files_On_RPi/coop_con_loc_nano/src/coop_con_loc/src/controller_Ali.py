...
#			yd_StP = pos_NED_m[1] + (0.1 * (time_traject - 40.0))
		...
		if not data_NED:
		        print(colored("Controller-Ali: No estimated position and velocity.","red"))
		else:
		...
		if data_NED_received:
			...
			ux_cmd = - (kv_x * ev_x + kvd_x * evd_x) # + kvi_x * evi_x)
			uy_cmd = + (kv_y * ev_y + kvd_y * evd_y) # + kvi_y * evi_y)
			uz_cmd = - (kv_z * ev_z + kvd_z * evd_z) # + kvi_z * evi_z)

			thrust_cmd = math.sqrt( (ux_cmd*ux_cmd) + (uy_cmd*uy_cmd) + (uz_cmd*uz_cmd) )
			roll_cmd = math.asin(+uy_cmd/thrust_cmd)
			pitch_cmd = math.atan2(-ux_cmd,uz_cmd)
			yaw_cmd = 0.0
