...
Results_File_EKF = open(r"/root/logs/Results_File_EKF.txt","w+")
Results_File_ACC = open(r"/root/logs/Results_File_ACC.txt","w+")
Results_File_NED = open(r"/root/logs/Results_File_NED.txt","w+")
...
def px4_communication():
	...
	pub_quat_data = rospy.Publisher("PX4_Orient", String, queue_size=1)
	publish_data_quat = String()
	rate = rospy.Rate(120)
	...
	while not rospy.is_shutdown():		
		...
		PX4_CSS_msg = PX4_connection.recv_match(type='ATTITUDE_QUATERNION',blocking=False) #HIGHRES_IMU -  CONTROL_SYSTEM_STATE
		if not PX4_CSS_msg:
			print(colored("PX4 IMU data: no message is received!","red"))
		else:
#			quat_x = PX4_CSS_msg.q[0]
#			quat_y = PX4_CSS_msg.q[1]
#			quat_z = PX4_CSS_msg.q[2]
#			quat_w = PX4_CSS_msg.q[3]
			quat_x = PX4_CSS_msg.q1
			quat_y = PX4_CSS_msg.q2
			quat_z = PX4_CSS_msg.q3
			quat_w = PX4_CSS_msg.q4
			data_quat = {'quat_x':quat_x,'quat_y':quat_x,'quat_z':quat_z,'quat_w':quat_w}
			publish_data_quat.data = json.dumps(data_quat)
			pub_quat_data.publish(publish_data_quat)
			print(colored(("estimated quaternions: qx=%.3f , qy=%.3f , qz=%.3f , qw=%.3f" % (quat_x,quat_y,quat_z,quat_w)),"green"))
