...
Results_File_Diff_DIST = open(r"/root/logs/Results_File_Diff_DIST.txt","w+")
Results_File_delta = open(r"/root/logs/Results_File_delta.txt","w+")
Results_File_Diff_delta = open(r"/root/logs/Results_File_Diff_delta.txt","w+")
...
def absolute_velocity():
	...
	pub_Vel_NED = rospy.Publisher("Vel_NED", String, queue_size=10)
	...
	while not rospy.is_shutdown():
		...
		rospy.Subscriber("Pos_UWB", String, callback_Pos_UWB, queue_size=10)
		if not data_Pos_UWB:
			print("No estimated UWB position is received!")
		else:
			Pos_UWB_X[0] = data_Pos_UWB.get("x_UWB_mm")
			Pos_UWB_X[1] = data_Pos_UWB.get("y_UWB_mm")
			Pos_UWB_X[2] = data_Pos_UWB.get("z_UWB_mm")			
		...
		print(colored(("Frequency of velocity estimation: %f Hz" %Frequency),"blue"))
