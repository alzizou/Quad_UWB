...
// Anchors at HumanITAS office (A-D-F-G)
long double vel_All_Anchors_lower_01[vel_Num_Anchors_lower_01][3] = {
    {-500.0,0.0,250.0},
    {4880.0,0.0,250.0},
    {0.0,5920.0,280.0},
    {5590.0,5790.0,400.0},
...
void vel_mat_dot1_lower_01(long double *inp1,long double *inp2,long double *out,int n_inp,int m_inp,int p_inp){
...
void vel_mat_scl1_lower_01(long double *inp, long double scl, long double *out, int n_inp, int m_inp){
    for (int i=0;i<n_inp;i++){
...
void vel_mat_add1_lower_01(long double *inp1, long double *inp2, long double *out, int n_inp, int m_inp){
    for (int i=0;i<n_inp;i++){
	for (int j=0;j<m_inp;j++){
...
void vel_mat_trn1_lower_01(long double *inp, long double *out, int n_inp, int m_inp){
    for (int i=0;i<n_inp;i++){
	for (int j=0;j<m_inp;j++){
...
void vel_mat_cns1_lower_01(long double *inp, long double *out, int n_inp, int m_inp){
    for (int i=0;i<n_inp;i++){
	for (int j=0;j<m_inp;j++){
...
long double vel_mat_nrm1_lower_01(long double *inp, int n_inp){
    long double out = 0.0;
    for (int i=0;i<n_inp;i++){
	out = out + (inp[i]*inp[i]);
    };
...
void vel_mat_srt_ind1_lower_01(long double *inp, int *out, int n_inp){
    long double *var = inp;
    for (int i=0;i<n_inp;i++){
...
long double vel_abs_ali1_lower_01(long double inp){
    long double out = inp;
    if (inp < 0.0){
	out = -1.0 * out;
    }
...
void velocitization(long double *Diff_DIST_inp, long double *DIST_inp, long double *Diff_delta_inp, long double *Pos_UWB_inp, long double *vel_out){
    ...
    long double vel_All_Dist[vel_Num_Anchors_lower_01][1];
    vel_mat_cns1_lower_01(DIST_inp,*vel_All_Dist,vel_Num_Anchors_lower_01,1);
    ...
    const int vel_Avail_Num_Anchors = vel_Num_Anchors_lower_01 - vel_num_init;
	...
	if (vel_All_Dist[i][0]>1.0){
	    ...
	    vel_Avail_All_Anchors[vel_num1][0] = vel_All_Anchors_lower_01[i][0];
	    vel_Avail_All_Anchors[vel_num1][1] = vel_All_Anchors_lower_01[i][1];
	    vel_Avail_All_Anchors[vel_num1][2] = vel_All_Anchors_lower_01[i][2];
	};
    };
	...
	vel_num = vel_num+1;
	vel_mat_dot1_lower_01(*vel_A_cpp,*V_old,*vel_B_hat_init,vel_Avail_Num_Anchors,3,1);
	vel_mat_scl1_lower_01(*vel_B_hat_init,-1.0,*vel_B_hat,vel_Avail_Num_Anchors,1);
	vel_mat_add1_lower_01(*vel_B,*vel_B_hat,*vel_e,vel_Avail_Num_Anchors,1);
	vel_mat_dot1_lower_01(*vel_alpha,*vel_A_cpp_trn,*V_dot_init1,3,3,vel_Avail_Num_Anchors);
	vel_mat_dot1_lower_01(*V_dot_init1,*vel_Weights,*V_dot_init2,3,vel_Avail_Num_Anchors,vel_Avail_Num_Anchors);
	vel_mat_dot1_lower_01(*V_dot_init2,*vel_e,*V_dot,3,vel_Avail_Num_Anchors,1);
	V[0][0] = V_old[0][0] + V_dot[0][0];
	V[1][0] = V_old[1][0] + V_dot[1][0];
	V[2][0] = V_old[2][0] + V_dot[2][0];
	vel_err = vel_mat_nrm1_lower_01(*vel_e,vel_Avail_Num_Anchors);
	vel_err_Arr = vel_err;
	if (vel_num>1){
	    vel_Dif_err = vel_err_Arr - vel_err_Arr_old;
	    vel_Abs_Dif_err = vel_abs_ali1_lower_01(vel_Dif_err);
	};
