...
Client_MySelf.bind(('192.168.0.58',8100))
Client_MySelf.settimeout(0.1)

Client_Neighbor1 = ('192.168.0.160',8100)
print(colored("UDP connection with the neighbor is provided.","blue"))

Results_File_RelVel = open(r"/root/coop_con_loc_nano/src/coop_con_loc/src/Results_File_RelVel.txt","w+")
...
def relative_velocity_leader():

	rospy.init_node("relative_velocity_leader", anonymous=True)
	rate = rospy.Rate(30)
	...
	while not rospy.is_shutdown():
		...
		if not data_NED:
			...
			try:
				...
				print("Received Data from the Follower: vx=%.3f , vy=%.3f , vz=%.3f" % (vel_Neighbor1_x,vel_Neighbor1_y,vel_Neighbor1_z))
				...
				print("Relative local velocity to the Follower: vx=%.3f , vy=%.3f , vz=%.3f" % (RelVel_x_mps,RelVel_y_mps,RelVel_z_mps))
