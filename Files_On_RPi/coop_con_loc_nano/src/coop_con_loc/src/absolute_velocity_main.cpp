...
long double vel_All_Anchors1_main[vel_Num_Anchors][3] = {
    {-500.0,0.0,250.0},
    {4880.0,0.0,250.0},
    {0.0,5920.0,280.0},
    {5590.0,5790.0,400.0},
...
//	std::cout << "Cpp Velocitization Frequency (Hz):" << ' ' << frequency<< std::endl;
	...
	for (int i=0;i<vel_Num_Anchors;i++){
	    delta_Euclidean[i][0] = ( pow((Pos_UWB_X[0][0] - vel_All_Anchors1_main[i][0]),2) + pow((Pos_UWB_X[1][0] - vel_All_Anchors1_main[i][1]),2) + pow((Pos_UWB_X[2][0] - vel_All_Anchors1_main[i][2]),2) ) - (pow(DIST[i][0],2));
	};
