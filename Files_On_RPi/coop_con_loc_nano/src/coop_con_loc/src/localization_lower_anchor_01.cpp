...
long double All_Anchors_lower_01[Num_Anchors_lower_01][3] = {
    {-500.0,0.0,250.0},
    {4880.0,0.0,250.0},
    {0.0,5920.0,280.0},
    {5590.0,5790.0,400.0},
...
void mat_dot1_lower_01(long double *inp1,long double *inp2,long double *out,int n_inp,int m_inp,int p_inp){
...
void mat_scl1_lower_01(long double *inp, long double scl, long double *out, int n_inp, int m_inp){
    for (int i=0;i<n_inp;i++){
...
void mat_add1_lower_01(long double *inp1, long double *inp2, long double *out, int n_inp, int m_inp){
    for (int i=0;i<n_inp;i++){
	for (int j=0;j<m_inp;j++){
...
void mat_trn1_lower_01(long double *inp, long double *out, int n_inp, int m_inp){
    for (int i=0;i<n_inp;i++){
	for (int j=0;j<m_inp;j++){
...
void mat_cns1_lower_01(long double *inp, long double *out, int n_inp, int m_inp){
    for (int i=0;i<n_inp;i++){
	for (int j=0;j<m_inp;j++){
...
long double mat_nrm1_lower_01(long double *inp, int n_inp){
    long double out = 0.0;
    for (int i=0;i<n_inp;i++){	
	out = out + (inp[i]*inp[i]);	
    };
...
void mat_srt_ind1_lower_01(long double *inp, int *out, int n_inp){
    long double *var = inp;
    for (int i=0;i<n_inp;i++){
...
long double abs_ali1_lower_01(long double inp){
    long double out = inp;
    if (inp < 0.0){
	out = -1.0 * out;
    }
...
void localization(long double *DIST_inp, long double *X_out){
    ...
    long double All_Dist[Num_Anchors_lower_01][1];
    mat_cns1_lower_01(DIST_inp, *All_Dist,Num_Anchors_lower_01,1);
    ...
    const int Avail_Num_Anchors = Num_Anchors_lower_01 - num;
	...
	if (All_Dist[i][0]>1.0){
	    ...
	    Avail_All_Anchors[num1][0] = All_Anchors_lower_01[i][0];
	    Avail_All_Anchors[num1][1] = All_Anchors_lower_01[i][1];
	    Avail_All_Anchors[num1][2] = All_Anchors_lower_01[i][2];
	};	 
    };
    ...
    mat_srt_ind1_lower_01(*Avail_All_Dist_Ind, Range_Ref_Anchors, (Avail_Num_Anchors-1));
	...
	while (Abs_Dif_err>Thresh){
	    num = num+1;
	    mat_dot1_lower_01(*A_cpp,*P_old,*B_hat_init,(Avail_Num_Anchors-1),3,1);
	    mat_scl1_lower_01(*B_hat_init,-1.0,*B_hat,(Avail_Num_Anchors-1),1);
	    mat_add1_lower_01(*B,*B_hat,*e,(Avail_Num_Anchors-1),1);
	    mat_dot1_lower_01(*alpha,*A_cpp_trn,*P_dot_init1,3,3,(Avail_Num_Anchors-1));
	    mat_dot1_lower_01(*P_dot_init1,*Weights,*P_dot_init2,3,(Avail_Num_Anchors-1),(Avail_Num_Anchors-1));
	    mat_dot1_lower_01(*P_dot_init2,*e,*P_dot,3,(Avail_Num_Anchors-1),1);
	    P[0][0] = P_old[0][0] + P_dot[0][0];
	    P[1][0] = P_old[1][0] + P_dot[1][0];
	    P[2][0] = P_old[2][0] + P_dot[2][0];	    
	    err = mat_nrm1_lower_01(*e, (Avail_Num_Anchors-1));
	    err_Arr = err;
	    if (num>1){
		Dif_err = err_Arr - err_Arr_old;
		Abs_Dif_err = abs_ali1_lower_01(Dif_err);
	    };	    	    
	    if (num>brk_itr_num){
		if (Dif_err>0.0){
		    P[0][0] = P_old[0][0];
